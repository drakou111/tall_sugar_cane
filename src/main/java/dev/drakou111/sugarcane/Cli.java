    ...
    private static final Command[] COMMANDS = {
            ...
            new Command("crossfind",
                    "[seeds] [threads] [targetHeight] [minA] [minB] [--dx=1] [--dz=0] "
                            ...
                            + "[--enum] [--enum-lows=8] [--enum-y=16:36] "
                            + "[--enum-from=<k>] [--enum-k=<n>] [--exact-carve]",
                    "Search for cross-chunk stacks, where `crosschunk` only measures them. "
                            ...
                            + "`crossmerge`. "
                            + "--enum fills pass 1 by CONSTRUCTING the RNG states that yield "
                            + "a wanted y rather than scanning decoration seeds for one, so "
                            + "the y costs nothing instead of being a 1-in-126 test. At "
                            + "height 10 that is 621 chains stored per second against the "
                            + "scan's 94, and every chain it returns is real where a third "
                            + "of the scan's are -- which matters more than the rate, since "
                            + "joins go as the SQUARE of the table size. It applies to pass 1 "
                            + "only, that being the rarer and dearer side. It is a SAMPLE, "
                            + "not a sweep of everything: k and the y band are covered in "
                            + "full but the state's low 17 bits are sampled --enum-lows of "
                            + "131072 ways, so the default sees 6.1e-5 of the band. Raise "
                            + "--enum-lows for more ground at linear cost; sweeps nest, so a "
                            + "wider one redoes everything a narrower one did. --enum-y is "
                            + "the ANCHOR invocation's y, not the chain's base, so it wants "
                            + "to be wider than the base band: at the default 84.9% of "
                            + "enumerated seeds survive the stored side's geometry filter. "
                            + "--enum-from and --enum-k split k between collaborators, the "
                            + "way --sample-from splits sample space; left off, it continues "
                            + "past whatever sweeps of the same shape --table already holds. "
                            + "--exact-carve answers the AIR carvers' water guard from the "
                            + "noise instead of stubbing it to false. Those carvers are the "
                            + "only source of air below sea level and they are exactly the "
                            + "ones that run the guard, so stubbed out a ravine carves "
                            + "through water the real one stops at -- which is why 58.6% of "
                            + "candidates died at chunk A's first column with the base not "
                            + "air. Measured against generated terrain over 332,800 blocks "
                            + "it loses no block the loose probe kept and cuts false "
                            + "positives 4,914 to 16. It is OFF by default because it costs "
                            + "about what it saves: 2.4x fewer chunk generations but 4.7% "
                            + "more wall clock on the same slice, since the guard fills a "
                            + "chunk of noise columns per sister. Turn it on when terrain "
                            + "dominates a run -- a large --table -- and off when streaming "
                            + "does. "
