...
The short version: the reverse search is ~3.8x more productive per candidate, the target build
at height 12 went from 12.5M to ~430M decoration seeds a second, and the search itself is 3.9x
faster on 24 threads. Most of that came from removing things that could not happen rather than
from going faster.

## The filter was wrong, and an independent implementation proved it

A collaborator contributed a standalone scanner for exact 4+4+4+4 stacks. Cross-checked against
our chain filter over 4M seeds, it found 2,616 and we found 3,512 — and its results were a
strict subset. **896 of ours, 26%, were chains no chunk could produce.**

Three rules closed the gap, each the same idea at a different scope:

- **Within an invocation.** All twenty tries share a y, so two tries at the same (x, z) are the
  same block. The first places; the second finds cane, not air.
- **Between invocations, for a continuation.** The earliest later invocation landing on the spot
  places there, at whatever height it drew. A later one cannot supply the column instead.
- **Between invocations, for the base.** The same, one step earlier.
...
Two implementations sharing no code, written from opposite directions, now agree exactly. Every
disagreement along the way was ours over-accepting.

**q(>=8) fell from 1.57% to 0.41%** — a 3.8x on the search, costing no coverage, because
everything removed was impossible.

## Search tall — a 16 costs the same as an 8

The terrain event a stack needs is not four columns each getting lucky. It is one ravine opening
a wall of air against water. Canyon vertical runs at our three tall finds: **5, 10 and 28
blocks**. Once the wall is there it satisfies every column at once.

So a height-8 search pays the same terrain cost per candidate and returns an 8. **Build your
target set at 16.** It is a strategy change, and it is free.

From height 8 up the carve probe now uses **ravines only** — every find at that height is
ravine-carved and no cave can open the vertical wall. Worth 31% of chunk generation.
`--all-carvers` puts them back; below height 8 they stay, since the confirmed 5-tall is
cave-carved.

## The target build

```
height 12, 100k targets:   33.9h  ->  6.4h
kernel at height 12:       12.5M  ->  ~430M seeds/s
GPU utilisation:           26-98% sawtooth  ->  94% mean
```

Three things, in order of what they were worth:

- **A dedicated N x 4 walk**, folded into `find_targets.cu` as a second kernel for heights
  divisible by four. No candidate arrays at all: it walks the stream once and skips an
  invocation's twenty tries with an O(1) LCG jump when the origin y cannot match. **11.3x at
  height 12**, byte-identical output.
- **Epoch sizing.** 50M samples was chosen when the filter ran at a few million a second; at
  ~390M/s an epoch had become 0.13 s of work wrapped in host work that does not shrink.
- **A ring over orbit neighbours.** For any decoration seed there is another whose stream is the
  first one's shifted by one invocation, so the filter keeps a ring and pays for one new
  invocation per seed instead of ten.

## The search

**3.9x on 24 threads**, and it was not the algorithm. `prepareBiomesOnly` built a fresh biome
source per sister with enlarged layer caches — an optimisation measured on the box scan, where
one source serves 1,400 chunks, applied where it serves a handful of lookups. 24 GB/s of
allocation, which is a bandwidth wall that does not scale with cores.

```
1 thread    28,053 -> 37,078 candidates/s
24 threads  84,031 -> 326,790
scaling     3.0x   -> 8.8x
```
...
- **`spot <relX> <relZ> <baseY> <height>`** — decoration seeds that grow a stack at one *named*
  ...
  matched to terrain you have verified.
...
- **Target builds start at a random sample index**, printed so a run can be repeated with
  `--sample-from`. Starting everyone at 0 meant two people building the same set did the same
  work.
- **Heights above 16** are reachable: a fifth shift level, and the packed chain re-laid out so a
  fifth column no longer overwrites its own shift fields. Asking for 17 used to accept nothing
  at all, forever, without an error.
- **`--max-slack`, `--shift-levels`, `--sample-from`, `--all-carvers`** — all in the GUI, along
  with tabs for `spot`, `merge` and `crosschunk`.
- **EXPLAINER.md**, the whole thing written for someone who has never seen the code.

## Fixes

- `--update` was ignored in the reverse search, which printed every five seconds whatever you
  asked for. Progress is also no longer one line per 50M-sample epoch.
- The bundled CUDA kernel now unpacks to a stable path and explains itself when antivirus blocks
  it, instead of silently dropping to a several-times-slower CPU path.
- `sisters` no longer prints a seed when it found nothing — it was naming whichever thread
  finished last — and says which kind of nothing it found.
- The slots tab no longer crashes on open.

## Known

`--water-probe` stays off: it rejects the reported 11-tall. Its water comes from neither the
noise fill nor a liquid carver, and until that source is identified any water prefilter silently
discards real finds. That is the largest thing open — 84% of the search is nine-chunk generation
to answer a question about five columns, and this is what blocks reading them directly.
